...
const AdminDashboard = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get('https://donjerseyssporthouseserver-71ee.onrender.com/categories');
  ...
  const handleCategoryClick = (categoryId) => {
    ...
    setLoading(true);
    axios
      .get(`https://donjerseyssporthouseserver-71ee.onrender.com/categories/${categoryId}`)
      .then((response) => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await axios.get('https://donjerseyssporthouseserver-71ee.onrender.com/orders');
  ...
  const handleDeleteProduct = async (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        await axios.delete(`https://donjerseyssporthouseserver-71ee.onrender.com/products/${productId}`);
  ...
  const handleDeleteOrder = async (orderId) => {
    try {
      ...
      const response = await axios.delete(`https://donjerseyssporthouseserver-71ee.onrender.com/orders/${orderId}`);
  ...
  const handleViewOrder = async (orderId) => {
    try {
      setLoadingOrders(true);
      const orderResponse = await axios.get(`https://donjerseyssporthouseserver-71ee.onrender.com/orders/${orderId}`);
